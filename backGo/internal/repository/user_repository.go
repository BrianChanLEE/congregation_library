...
func (r *UserRepository) Create(user *models.User) error {
	...
	_, err := db.DB.Exec(query, user.Name, user.JWhubEmail, user.PasswordHash, user.Status)
	return err
}
...
func (r *UserRepository) GetByID(id int64) (*models.User, error) {
	...
	err := db.DB.QueryRow(query, id).Scan(&user.ID, &user.Name, &user.JWhubEmail, &user.Role, &user.Status)
	if err != nil {
		return nil, err
	}
	return &user, nil
...
func (r *UserRepository) GetByJwhubEmailAndCongID(email string, congID int64) (*models.User, error) {
	...
	var user models.User
	err := db.DB.QueryRow(query, email, congID).Scan(&user.ID, &user.Name, &user.JWhubEmail, &user.PasswordHash, &user.Role)
	if err != nil {
		return nil, err
	}
	return &user, nil
...
func (r *UserRepository) UpdateStatus(userID int64, status string) error {
	...
	_, err := db.DB.Exec(query, status, userID)
	return err
}
...
func (r *UserRepository) GetPasswordHash(userID int64) (string, error) {
	...
	var hash string
	err := db.DB.QueryRow(query, userID).Scan(&hash)
	return hash, err
}
...
func (r *UserRepository) UpdatePassword(userID int64, hashedNewPassword string) error {
	...
	_, err := db.DB.Exec(query, hashedNewPassword, userID)
	return err
}
